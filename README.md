...
* **Backend (Amazon EC2):**
    The Node.js backend is deployed on an Amazon EC2 instance. Ensure the EC2 instance has Node.js and npm installed. Configure your EC2 security groups to allow inbound traffic on the port your backend server is listening on. You might use a process manager like PM2 to keep your Node.js application running.

* **Frontend (Render):**
    The React frontend is deployed on Render. Connect your Git repository to Render and configure it to build and serve the static assets generated by `npm run build` (typically from the `dist` directory). Ensure environment variables (like `VITE_API_BASE_URL` pointing to your live EC2 backend) are set up in the Render dashboard.

---

This README provides a comprehensive overview of your project. You can copy and paste this into a `README.md` file in the root of your project repository.
